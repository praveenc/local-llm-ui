...
- `docs/AISDK-LEARNINGS.md` — tells you the architecture, providers, file structure, and known patterns.
- `docs/SECURITY-LEARNINGS.md` — tells you what's already been found, fixed, and deferred. Use SEC-XX IDs for cross-referencing. New findings should use the next available SEC-XX number (check the Audit History table).
...
At the start of every review, check availability and set fallback:
```bash
...
```bash
# Search TypeScript/TSX files (case-insensitive)
...
Run `npm audit` via Bash to check for known vulnerabilities.
...
## Rules
1. Use `rg` (ripgrep) via Bash for all code searches — faster and respects .gitignore
