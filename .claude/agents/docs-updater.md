...
- **Project Structure** tree must match actual files — verify with Glob before updating
...
2. **Never duplicate content** — use Grep to search for existing coverage of the topic before adding
...
6. **Preserve section ordering** — new content goes in the appropriate section:
   ...
   - **AISDK-LEARNINGS.md**: Provider learnings in Provider-Specific area; pitfalls in Common Pitfalls (numbered sequentially)
   - **SECURITY-LEARNINGS.md**: Findings in appropriate domain section; pitfalls in Common Pitfalls (numbered); new audits in Audit History table
...
9. **Use `rg` (ripgrep)** via Bash to verify file paths and code patterns referenced in docs are accurate — fall back to `grep -rn` if rg is unavailable
