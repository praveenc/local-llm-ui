...
Before starting any investigation, read `docs/AISDK-LEARNINGS.md`.
...
```bash
grep -rn 'pattern' src/components/ --include='*.ts' --include='*.tsx'
```
...
Use `rg` to find the relevant component files:
```bash
