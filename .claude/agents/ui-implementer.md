...
Before starting any implementation, read the project learnings file: `docs/AISDK-LEARNINGS.md`
...
If you're unsure about a security implication, flag it in your output with `SECURITY: [concern]` so the calling agent can run `security-reviewer`.
...
5. Use `rg` (ripgrep) via Bash for searching code patterns: `rg -n 'pattern' src/` — fall back to `grep -rn 'pattern' src/ --include='*.ts' --include='*.tsx'` if rg is unavailable
...
- SECURITY: [flag for security-reviewer]
