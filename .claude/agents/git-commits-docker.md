...
3. **Before committing, always verify** no sensitive files are staged:
   ...
   ```
   If sensitive files are detected, STOP and report the issue.
4. **NEVER force-push** (`git push --force` or `--force-with-lease`) unless the user explicitly says "force push" with a reason.
